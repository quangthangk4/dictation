...
export default async function DictationPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 mt-6 lg:mt-10">
      ...
      {dictation.audioUrl && (
        <div className="mb-8 glass-panel p-4 rounded-2xl border border-slate-200/60 shadow-sm flex items-center justify-between">
          <div className="flex items-center gap-3 text-slate-600 font-semibold text-sm">
            <svg className="w-6 h-6 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3" /></svg>
            Listening Track
          </div>
          <audio controls src={dictation.audioUrl} className="w-full max-w-md" />
        </div>
      )}
